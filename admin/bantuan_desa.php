<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["aksi"])) {
            ...
            if ($aksi == "tambah") {
                $query = "INSERT INTO bantuan (nama_penerima, tanggal_diterima, jenis) VALUES ('$nama_penerima', '$tanggal_diterima', '$jenis')";
            } elseif ($aksi == "edit" && $id) {
                $query = "UPDATE bantuan SET nama_penerima='$nama_penerima', tanggal_diterima='$tanggal_diterima', jenis='$jenis' WHERE id='$id'";
            ...
            if (isset($query)) {
                ...
                header("Location: bantuan_desa.php?page=bantuan_desa");
                exit;
            }
        }
    }
...
<body>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th><button class="fw-bold p-0 btn sort-btn" data-key="nama_penerima">Nama Penerima</button></th>
                    <th><button class="fw-bold p-0 btn sort-btn" data-key="tanggal_diterima">Tanggal Diterima</button></th>
                    <th><button class="fw-bold p-0 btn sort-btn" data-key="jenis">Jenis</button></th>
            ...
            <tbody id="tableBody">
                <?php 
                    ...
                    ?>
                        <tr class="table-row">
                            <td><?= $no?></td>
                            <td class="search-target"><?= $row["nama_penerima"]?></td>
                            <td><?= $row["tanggal_diterima"]?></td>
                            <td class="search-target"><?= $row["jenis"]?></td>
                            <td>
                        ...
                        <div class="modal fade" id="modalEdit<?= $row['id'] ?>" tabindex="-1" aria-labelledby="modalEditLabel<?= $row['id'] ?>" aria-hidden="true">
                            <div class="modal-dialog">
                                ...
                                <form method="POST">
                                    ...
                                    <h5 class="modal-title" id="modalEditLabel<?= $row['id'] ?>">Edit Bantuan</h5>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control" name="tanggal_diterima" value="<?= $row['tanggal_diterima'] ?>" required>
                                    </div>
                                    <div class="mb-3">
                        ...
                        <div class="modal fade" id="modalDelete<?= $row['id'] ?>" tabindex="-1" aria-labelledby="modalDeleteLabel<?= $row['id'] ?>" aria-hidden="true">
                            <div class="modal-dialog">
                                ...
                                <form method="POST">
                                    ...
                                    <input type="hidden" name="id" value="<?= $row['id'] ?>">
                                    <div class="modal-header">
                                    <h5 class="modal-title" id="modalDeleteLabel<?= $row['id'] ?>">Hapus Bantuan</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                                    </div>
                                    <div class="modal-body">
                                    <p>Apakah kamu yakin ingin menghapus <strong><?= $row['nama_penerima'] ?></strong>?</p>
                                    </div>
                                    <div class="modal-footer">
    ...
    <div class="modal fade" id="modalTambah" tabindex="-1" aria-labelledby="modalTambahLabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <form method="POST">
                ...
                <div class="modal-header">
                <h5 class="modal-title" id="modalTambahLabel">Tambah Bantuan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                </div>
                <div class="modal-body">
                <div class="mb-3">
    ...
    <script>
        ...
        function getColumnIndex(key) {
            if (key === 'nama_penerima') return 1;
            if (key === 'tanggal_diterima') return 2;
            if (key === 'jenis') return 3;
            return 1;
        }
    </script>
