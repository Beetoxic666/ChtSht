<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["aksi"])) {
            ...
            if ($aksi == "tambah") {
                $query = "INSERT INTO struktur (nama, jabatan, foto) VALUES ('$nama', '$jabatan', '$foto')";
            } elseif ($aksi == "edit" && $id) {
                $query = "UPDATE struktur SET nama='$nama', jabatan='$jabatan', foto='$foto' WHERE id='$id'";
            ...
            if (isset($query)) {
                ...
                header("Location: struktur_desa.php?page=struktur_desa");
                exit;
            }
        }
    }
...
<body>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th><button class="fw-bold p-0 btn sort-btn" data-key="nama">Nama</button></th>
                    <th><button class="fw-bold p-0 btn sort-btn" data-key="jabatan">Jabatan</button></th>
                    <th><button class="fw-bold p-0 btn">Foto</button></th>
                ...
                <?php 
                    ...
                    ?>
                        <tr class="table-row">
                            ...
                            <td><?= $row["jabatan"]?></td>
                            <td><img src="../assets/<?= $row['foto']?>" alt="" width="150" height="150"></td>
                            <td>
                        ...
                        <div class="modal fade" id="modalEdit<?= $row['id'] ?>" tabindex="-1" aria-labelledby="modalEditLabel<?= $row['id'] ?>" aria-hidden="true">
                            <div class="modal-dialog">
                                ...
                                <form method="POST">
                                    ...
                                    <h5 class="modal-title" id="modalEditLabel<?= $row['id'] ?>">Edit Struktur</h5>
                        ...
                        <div class="modal fade" id="modalDelete<?= $row['id'] ?>" tabindex="-1" aria-labelledby="modalDeleteLabel<?= $row['id'] ?>" aria-hidden="true">
                            <div class="modal-dialog">
                                ...
                                <form method="POST">
                                    ...
                                    <input type="hidden" name="id" value="<?= $row['id'] ?>">
                                    <div class="modal-header">
                                    <h5 class="modal-title" id="modalDeleteLabel<?= $row['id'] ?>">Hapus Struktur</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                                    </div>
                                    <div class="modal-body">
                                    <p>Apakah kamu yakin ingin menghapus <strong><?= $row['nama'] ?></strong>?</p>
                                    </div>
                                    <div class="modal-footer">
    ...
    <div class="modal fade" id="modalTambah" tabindex="-1" aria-labelledby="modalTambahLabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <form method="POST">
                ...
                <div class="modal-header">
                <h5 class="modal-title" id="modalTambahLabel">Tambah Struktur</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                </div>
                <div class="modal-body">
                <div class="mb-3">
    ...
    <script>
        ...
        function getColumnIndex(key) {
            if (key === 'nama') return 1;
            if (key === 'jabatan') return 2;
            return 1;
        }
    </script>
