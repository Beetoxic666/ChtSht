<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["aksi"])) {
            ...
            if ($aksi == "tambah") {
                $query = "INSERT INTO info_desa (info, jenis) VALUES ('$info', 'geografis')";
            } elseif ($aksi == "edit" && $id) {
                $query = "UPDATE info_desa SET info='$info' WHERE id='$id'";
            } elseif ($aksi == "hapus" && $id) {
                $query = "DELETE FROM info_desa WHERE id='$id'";
            }

            if (isset($query)) {
                $result = mysqli_query($conn, $query);
                header("Location: geografis.php?");
                exit;
            }
        }
    }
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th><button class="fw-bold p-0 btn">Berikut ini adalah kondidi geografis Desa Rimba Beringin</button></th>
            ...
            <tbody id="tableBody">
                <?php 
                    $no = 1;
                    $query = "SELECT * FROM info_desa WHERE jenis='geografis'";
                    $result = mysqli_query($conn, $query);
                            ...
                            <div class="modal-dialog">
                                ...
                                <form method="POST">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" name="info" required><?= $row['info'] ?></textarea>
                                    </div>
                                    </div>
                        ...
                        <div class="modal fade" id="modalDelete<?= $row['id'] ?>" tabindex="-1" aria-labelledby="modalDeleteLabel<?= $row['id'] ?>" aria-hidden="true">
                            <div class="modal-dialog">
                                ...
                                <form method="POST">
                                    ...
                                    <div class="modal-body">
                                    <p>Apakah kamu yakin ingin menghapus <strong><?= $row['id'] ?></strong>?</p>
                                    </div>
                                    <div class="modal-footer">
